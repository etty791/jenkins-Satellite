pipeline {
  ...
  stages {
        ...
        stage('Analyze fuel status') {
          steps {
            retry(2) {
                ...
                bat '''
                if not exist fuelFail.marker (
                echo First attempt failed > fuelFail.marker
                exit /b 1
                )
                echo Step passed
                '''
    ...
    stage('Secure Deployment Preparation') {
      ...
      when {
        expression { env.GIT_BRANCH == 'origin/main' and env.APP_ENV == 'staging' }
      }
    ...
    stage('Deploy Satellite Processing System') {
      steps {
        ...
        archiveArtifacts artifacts: 'results/report.xml'
      }
